...
echo "== Fase 40.2: soporte visual/exportable de curvas de patronaje MVP =="
echo "== Rol operativo =="
echo "Arquitecto/consultor tecnico del producto + experto en patronaje."
echo "Criterio: curvas MVP, sin vender resultado como patron industrial definitivo."
...
"""Visual curve annotations for MVP pattern exports."""
...
def _pants_curves(piece: PatternPiece) -> list[VisualCurve]:
    ...
    if top_right_candidates and bottom_right_candidates:
...
if "def _curve_elements" not in text:
    marker = "def _dimension_elements(tx: Any, ty: Any, dim: dict[str, Any]) -> list[str]:\n"
    if marker not in text:
        raise SystemExit("ERROR: no se encontro _dimension_elements en SVG writer")
    text = text.replace(marker, curve_func + marker, 1)
...
if "get(\"visual_curves\"" not in text:
    ...
    if marker not in text:
        raise SystemExit("ERROR: no se encontro bloque de dimensiones SVG")
    text = text.replace(
        marker,
...
if "def _draw_curve" not in text:
    marker = "def _draw_dimension(canvas: Any, tx: Any, ty: Any, dim: dict[str, Any], scale: float) -> None:\n"
    if marker not in text:
        raise SystemExit("ERROR: no se encontro _draw_dimension en PDF writer")
    text = text.replace(marker, curve_func + marker, 1)

if "get(\"visual_curves\"" not in text:
    marker = "        for dim in (piece.metadata or {}).get(\"dimension_annotations\", []):\n            _draw_dimension(c, tx, ty, dim, scale)\n"
    if marker not in text:
        raise SystemExit("ERROR: no se encontro bloque de dimensiones PDF")
    text = text.replace(
        marker,
...
echo "== Fase 40.2 aplicada correctamente =="
